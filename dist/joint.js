/*! JointJS v2.2.1 (2019-08-22) - JavaScript diagramming library
...
(function (global, factory) {
    typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('lodash'), require('jquery'), require('dagre'), require('backbone')) :
    typeof define === 'function' && define.amd ? define(['exports', 'lodash', 'jquery', 'dagre', 'backbone'], factory) :
    (global = global || self, factory(global.joint = {}, global._, global.$, global.dagre, global.Backbone));
}(this, function (exports, _, $, dagre, Backbone) { 'use strict';
    ...
    var bezier = {
      ...
      curveThroughPoints: function curveThroughPoints(points) {
      ...
      getCurveControlPoints: function getCurveControlPoints(knots) {
        console.warn('deprecated');
      ...
      getCurveDivider: function getCurveDivider(p0, p1, p2, p3) {
        console.warn('deprecated');
      ...
      getFirstControlPoints: function getFirstControlPoints(rhs) {
        console.warn('deprecated');
      ...
      getInversionSolver: function getInversionSolver(p0, p1, p2, p3) {
        console.warn('deprecated');
        var curve = new Curve(p0, p1, p2, p3);
    ...
    var Curve = function Curve(p1, p2, p3, p4) {
    ...
    Curve.prototype = {
      ...
      closestPoint: function closestPoint(p, opt) {
        return this.pointAtT(this.closestPointT(p, opt));
      },
      closestPointLength: function closestPointLength(p, opt) {
        opt = opt || {};
      ...
      closestPointNormalizedLength: function closestPointNormalizedLength(p, opt) {
        opt = opt || {};
        var precision = opt.precision === undefined ? this.PRECISION : opt.precision;
        var subdivisions = opt.subdivisions === undefined ? this.getSubdivisions({
          precision: precision
      ...
      closestPointT: function closestPointT(p, opt) {
        opt = opt || {};
        var precision = opt.precision === undefined ? this.PRECISION : opt.precision;
        var subdivisions = opt.subdivisions === undefined ? this.getSubdivisions({
          precision: precision
      ...
      closestPointTangent: function closestPointTangent(p, opt) {
      ...
      containsPoint: function containsPoint(p, opt) {
        var polyline = this.toPolyline(opt);
        return polyline.containsPoint(p);
      },
      // Divides the curve into two at requested `ratio` between 0 and 1 with precision better than `opt.precision`; optionally using `opt.subdivisions` provided.
      // For a function that uses `t`, use Curve.divideAtT().
      divideAt: function divideAt(ratio, opt) {
      ...
      divideAtLength: function divideAtLength(length, opt) {
      ...
      divideAtT: function divideAtT(t) {
        var start = this.start;
        var controlPoint1 = this.controlPoint1;
        var controlPoint2 = this.controlPoint2;
      ...
      getSubdivisions: function getSubdivisions(opt) {
        opt = opt || {};
      ...
      lengthAtT: function lengthAtT(t, opt) {
        if (t <= 0) return 0;
        opt = opt || {};
        var precision = opt.precision === undefined ? this.PRECISION : opt.precision; // not using opt.subdivisions
        // not using localOpt
      ...
      pointAt: function pointAt(ratio, opt) {
      ...
      pointAtLength: function pointAtLength(length, opt) {
        var t = this.tAtLength(length, opt);
        return this.pointAtT(t);
      },
      ...
      // Default precision
      PRECISION: 3,
      scale: function scale(sx, sy, origin) {
      ...
      tangentAt: function tangentAt(ratio, opt) {
      ...
      tangentAtLength: function tangentAtLength(length, opt) {
        if (!this.isDifferentiable()) return null;
        var t = this.tAtLength(length, opt);
        return this.tangentAtT(t);
      },
      ...
      tAtLength: function tAtLength(length, opt) {
        var fromStart = true;
      ...
      toPoints: function toPoints(opt) {
        opt = opt || {};
        var precision = opt.precision === undefined ? this.PRECISION : opt.precision; // opt.precision only used in getSubdivisions() call
      ...
      // Flattened length is no more than 10^(-precision) away from real curve length.
      toPolyline: function toPolyline(opt) {
    ...
    var Ellipse = function Ellipse(c, a, b) {
    ...
    Ellipse.prototype = {
      ...
      equals: function equals(ellipse) {
      ...
      intersectionWithLineFromCenterToPoint: function intersectionWithLineFromCenterToPoint(p, angle) {
        p = new Point(p);
       ...
       */
      normalizedDistance: function normalizedDistance(point) {
        var x0 = point.x;
        var y0 = point.y;
        var a = this.a;
        var b = this.b;
       ...
       */
      tangentTheta: function tangentTheta(p) {
    ...
    Line.prototype = {
      ...
      closestPointLength: function closestPointLength(p) {
        return this.closestPointNormalizedLength(p) * this.length();
      },
      // @return {number} the normalized length of the closest point on the line to point `p`
      closestPointNormalizedLength: function closestPointNormalizedLength(p) {
      ...
      closestPointTangent: function closestPointTangent(p) {
      ...
      containsPoint: function containsPoint(p) {
        ...
        if (new g.Line(p, end).length() > length) return false; // else: `p` lies between start and end of the line

        return true;
      },
      // Divides the line into two at requested `ratio` between 0 and 1.
      divideAt: function divideAt(ratio) {
        var dividerPoint = this.pointAt(ratio); // return array with two new lines

        return [new Line(this.start, dividerPoint), new Line(dividerPoint, this.end)];
      },
      // Divides the line into two at requested `length`.
      divideAtLength: function divideAtLength(length) {
      ...
      intersect: function intersect(shape, opt) {
      ...
      pointAtLength: function pointAtLength(length) {
        var start = this.start;
        var end = this.end;
        var fromStart = true;
      ...
      pointOffset: function pointOffset(p) {
      ...
      rotate: function rotate(origin, angle) {
      ...
      round: function round(precision) {
        ...
        this.start.x = _round(this.start.x * f) / f;
        this.start.y = _round(this.start.y * f) / f;
        this.end.x = _round(this.end.x * f) / f;
        this.end.y = _round(this.end.y * f) / f;
        return this;
      },
      scale: function scale(sx, sy, origin) {
        this.start.scale(sx, sy, origin);
      ...
      tangentAtLength: function tangentAtLength(length) {
        if (!this.isDifferentiable()) return null;
        var start = this.start;
        var end = this.end;
    ...
    Path.prototype = {
      ...
      appendSegment: function appendSegment(arg) {
      ...
      getSegmentSubdivisions: function getSegmentSubdivisions(opt) {
        var segments = this.segments;
        var numSegments = segments.length; // works even if path has no segments
      ...
      // Throws an error if index is out of range.
      // Throws an error if argument is not a segment or an array of segments.
      insertSegment: function insertSegment(index, arg) {
        var segments = this.segments;
        var numSegments = segments.length; // works even if path has no segments
      ...
      intersectionWithLine: function intersectionWithLine(line, opt) {
      ...
      prepareSegment: function prepareSegment(segment, previousSegment, nextSegment) {
      ...
      // Throws an error if index is out of range.
      // Throws an error if argument is not a segment or an array of segments.
      replaceSegment: function replaceSegment(index, arg) {
        var segments = this.segments;
        var numSegments = segments.length;
        if (numSegments === 0) throw new Error('Path has no segments.');
        if (index < 0) index = numSegments + index; // convert negative indices to positive
      ...
      segmentAt: function segmentAt(ratio, opt) {
        var index = this.segmentIndexAt(ratio, opt);
        if (!index) return null;
        return this.getSegment(index);
      },
      // Accepts negative length.
      segmentAtLength: function segmentAtLength(length, opt) {
        var index = this.segmentIndexAtLength(length, opt);
        if (!index) return null;
        return this.getSegment(index);
      },
      segmentIndexAt: function segmentIndexAt(ratio, opt) {
        var segments = this.segments;
        var numSegments = segments.length;
        if (numSegments === 0) return null; // if segments is an empty array
      ...
      // Accepts negative length.
      segmentIndexAtLength: function segmentIndexAtLength(length, opt) {
        var segments = this.segments;
        var numSegments = segments.length;
        if (numSegments === 0) return null; // if segments is an empty array
      ...
      toPolylines: function toPolylines(opt) {
        var polylines = [];
      ...
      updateSubpathStartSegment: function updateSubpathStartSegment(segment) {
    ...
    */
    ...
    Point.prototype = {
      chooseClosest: function chooseClosest(points) {
        var n = points.length;
      ...
      adhereToRect: function adhereToRect(r) {
      ...
      angleBetween: function angleBetween(p1, p2) {
      ...
      bearing: function bearing(point) {
      ...
      cross: function cross(p1, p2) {
        return p1 && p2 ? (p2.x - this.x) * (p1.y - this.y) - (p2.y - this.y) * (p1.x - this.x) : NaN;
      },
      difference: function difference(dx, dy) {
        if (Object(dx) === dx) {
          dy = dx.y;
          dx = dx.x;
        }
      ...
      reflection: function reflection(ref) {
      ...
      round: function round(precision) {
        var f = pow(10, precision || 0);
        this.x = _round(this.x * f) / f;
        this.y = _round(this.y * f) / f;
        return this;
      },
      ...
      squaredDistance: function squaredDistance(p) {
      ...
      vectorAngle: function vectorAngle(p) {
        var zero = new Point(0, 0);
        return zero.angleBetween(this, p);
      }
    };
    Point.prototype.translate = Point.prototype.offset;
    var Polyline = function Polyline(points) {
    ...
    Polyline.prototype = {
      ...
      intersectionWithLine: function intersectionWithLine(l) {
        var line = new Line(l);
        var intersections = [];
        var points = this.points;
      ...
      pointAt: function pointAt(ratio) {
        var points = this.points;
        var numPoints = points.length;
        if (numPoints === 0) return null; // if points array is empty
      ...
      tangentAt: function tangentAt(ratio) {
        var points = this.points;
        var numPoints = points.length;
        if (numPoints === 0) return null; // if points array is empty
    ...
    Rect.prototype = {
      ...
      containsRect: function containsRect(r) {
      ...
      intersect: function intersect(r) {
      ...
      maxRectScaleToFit: function maxRectScaleToFit(rect, origin) {
        rect = new Rect(rect);
      ...
      maxRectUniformScaleToFit: function maxRectUniformScaleToFit(rect, origin) {
        var scale = this.maxRectScaleToFit(rect, origin);
        return min(scale.sx, scale.sy);
      },
      // Move and expand me.
      // @param r {rectangle} representing deltas
      moveAndExpand: function moveAndExpand(r) {
        this.x += r.x || 0;
        this.y += r.y || 0;
      ...
      pointNearestToPoint: function pointNearestToPoint(point) {
      ...
      round: function round(precision) {
        ...
        this.x = _round(this.x * f) / f;
        this.y = _round(this.y * f) / f;
        this.width = _round(this.width * f) / f;
        this.height = _round(this.height * f) / f;
        return this;
      },
      ...
      sideNearestToPoint: function sideNearestToPoint(point) {
        point = new Point(point);
    ...
    var scale = {
      ...
      linear: function linear(domain, range, value) {
    ...
    var _snapToGrid = function _snapToGrid(value, gridSize) {
      return gridSize * _round(value / gridSize);
    };
    var toDeg = function toDeg(rad) {
      return 180 * rad / PI % 360;
    };
    var toRad = function toRad(deg, over360) {
    ...
    var line = Line;
    var point = Point;
    var rect = Rect; // Local helper function.
    ...
    var segmentPrototype = {
      ...
      closestPointT: function closestPointT(p) {
      ...
      // virtual
      divideAt: function divideAt() {
        throw new Error('Declaration missing for virtual function.');
      },
      // virtual
      divideAtLength: function divideAtLength() {
        throw new Error('Declaration missing for virtual function.');
      },
    ...
    var V$1 = function () {
      var hasSvg = (typeof window === "undefined" ? "undefined" : _typeof(window)) === 'object' && !!(window.SVGAngle || document.implementation.hasFeature('http://www.w3.org/TR/SVG11/feature#BasicStructure', '1.1')); // SVG support is required.
      ...
      var V = function V(el, attrs, children) {
      ...
      VPrototype.translateAndAutoOrient = function (position, reference, target) {
        ...
        var angle = new Point(position).changeInAngle(position.x - reference.x, position.y - reference.y, reference);
        rotateAroundOrigin.setRotate(angle, 0, 0); // 3. Translate to the `position` + the offset (half my width) towards the `reference` point.

        var translateFinal = svg.createSVGTransform();
        var finalPosition = new Point(position).move(reference, bbox.width / 2);
        translateFinal.setTranslate(position.x + (position.x - finalPosition.x), position.y + (position.y - finalPosition.y)); // 4. Apply transformations.

        var ctm = this.getTransformToElement(target || svg);
        var transform = svg.createSVGTransform();
        transform.setMatrix(translateFinal.matrix.multiply(rotateAroundOrigin.matrix.multiply(translateToOrigin.matrix.multiply(ctm)))); // Instead of directly setting the `matrix()` transform on the element, first, decompose
        ...
        this.rotate(decomposition.rotation); // Note that scale has been already applied, hence the following line stays commented. (it's here just for reference).
      ...
      VPrototype.findIntersection = function (ref, target) {
        ...
        var spot;
        var tagName = this.tagName(); // Little speed up optimalization for `<rect>` element. We do not do conversion
        // to path element and sampling but directly calculate the intersection through
        // a transformed geometrical rectangle.

        if (tagName === 'RECT') {
          var gRect = new Rect(parseFloat(this.attr('x') || 0), parseFloat(this.attr('y') || 0), parseFloat(this.attr('width')), parseFloat(this.attr('height'))); // Get the rect transformation matrix with regards to the SVG document.
      ...
      V.convertLineToPathData = function (line$$1) {
        line$$1 = V(line$$1);
        var d = ['M', line$$1.attr('x1'), line$$1.attr('y1'), 'L', line$$1.attr('x2'), line$$1.attr('y2')].join(' ');
        return d;
      };
    ...
    var addClassNamePrefix = function addClassNamePrefix(className) {
    ...
    var removeClassNamePrefix = function removeClassNamePrefix(className) {
      if (!className) return className;
      return className.toString().split(' ').map(function (_className) {
        if (_className.substr(0, classNamePrefix.length) === classNamePrefix) {
    ...
    var parseDOMJSON = function parseDOMJSON(json, namespace) {
      var selectors = {};
      var groupSelectors = {};
      var svgNamespace = V$1.namespace.xmlns;
      ...
      while (queue.length > 0) {
        ...
        for (var i = 0, n = siblingsDef.length; i < n; i++) {
          ...
          var wrapper = svg ? V$1 : $; // Attributes
    ...
    var getByPath = function getByPath(obj, path, delim) {
    ...
    var setByPath = function setByPath(obj, path, value, delim) {
    ...
    var unsetByPath = function unsetByPath(obj, path, delim) {
      delim = delim || '/';
    ...
    var flattenObject = function flattenObject(obj, delim, stop) {
      delim = delim || '/';
      var ret = {};

      for (var key in obj) {
        if (!obj.hasOwnProperty(key)) continue;
        var shouldGoDeeper = _typeof(obj[key]) === 'object';
    ...
    var normalizeEvent = function normalizeEvent(evt) {
    ...
    var shapePerimeterConnectionPoint = function shapePerimeterConnectionPoint(linkView, view, magnet, reference) {
      var bbox;
      var spot;
      ...
      if (magnet) {
        spot = V$1(magnet).findIntersection(reference, linkView.paper.viewport);

        if (!spot) {
          bbox = V$1(magnet).getBBox({
    ...
    var isPercentage = function isPercentage(val) {
      return isString(val) && val.slice(-1) === '%';
    };
    var parseCssNumeric = function parseCssNumeric(val, restrictUnits) {
    ...
    var breakText = function breakText(text, size, styles, opt) {
      opt = opt || {};
      styles = styles || {};
    ...
    var downloadDataUri = function downloadDataUri(dataUri, fileName) {
    ...
    var imageToDataUri = function imageToDataUri(url, callback) {
      ...
      var modernHandler = function modernHandler(xhr, callback) {
        if (xhr.status === 200) {
      ...
      var legacyHandler = function legacyHandler(xhr, callback) {
    ...
    var getElementBBox = function getElementBBox(el) {
      var $el = $(el);
    ...
    var sortElements = function sortElements(elements, comparator) {
    ...
    var setAttributesBySelector = function setAttributesBySelector(element, attrs) {
      var $element = $(element);
    ...
    var normalizeSides = function normalizeSides(box) {
      if (Object(box) !== box) {
    ...
    var interpolate = {
      ...
      hexColor: function hexColor(a, b) {
    ...
    var format = {
      ...
      number: function number(specifier, value, locale) {
        locale = locale || {
          currency: ['$', ''],
          decimal: '.',
          thousands: ',',
      ...
      string: function string(formatString, value) {
      ...
      convert: function convert(type, value, precision) {
        switch (type) {
          case 'b':
      ...
      precision: function precision(value, _precision) {
        return _precision - (value ? Math.ceil(Math.log(value) / Math.LN10) : 1);
      },
      prefix: function prefix(value, precision) {
    ...
    */
    ...
    var toggleFullScreen = function toggleFullScreen(el) {
    ...
    var assign = _.assign;
    var defaults = _.defaults; // no better-named replacement for `deepMixin`
    ...
    var isBoolean = function isBoolean(value) {
      ...
      return value === true || value === false || !!value && _typeof(value) === 'object' && toString.call(value) === '[object Boolean]';
    };
    ...
    var isNumber = function isNumber(value) {
      var toString = Object.prototype.toString;
      return typeof value === 'number' || !!value && _typeof(value) === 'object' && toString.call(value) === '[object Number]';
    };
    var isString = function isString(value) {
      var toString = Object.prototype.toString;
      return typeof value === 'string' || !!value && _typeof(value) === 'object' && toString.call(value) === '[object String]';
    };
    var noop = function noop() {};
    var isPaper = function isPaper(view) {
      return view && view.isPaper && view.isPaper();
    };
    var isCellView = function isCellView(view) {
      return isLinkView(view) || isElementView(view);
    };
    var isLinkView = function isLinkView(view) {
      return view.model && view.model.isLink && view.model.isLink();
    };
    var isElementView = function isElementView(view) {
    ...
    function ellipseLayout(ports, elBBox, startAngle, stepFn) {
      ...
      return ports.map(function (port, index, ports) {
        ...
        var theta = port.compensateRotation ? -ellipse$$1.tangentTheta(p2) : 0; // `dx`,`dy` per port offset option
    ...
    var absolute = function absolute(ports, elBBox, opt) {
      //TODO v.talas angle
    ...
    var fn = function fn(ports, elBBox, opt) {
    ...
    var line$1 = function line$$1(ports, elBBox, opt) {
    ...
    var left = function left(ports, elBBox, opt) {
    ...
    var right = function right(ports, elBBox, opt) {
    ...
    var top = function top(ports, elBBox, opt) {
    ...
    var bottom = function bottom(ports, elBBox, opt) {
    ...
    var ellipseSpread = function ellipseSpread(ports, elBBox, opt) {
    ...
    var ellipse$1 = function ellipse$$1(ports, elBBox, opt) {
      var startAngle = opt.startAngle || 0;
    ...
    var manual = function manual(portPosition, elBBox, opt) {
      return labelAttributes(opt, elBBox);
    };
    var left$1 = function left(portPosition, elBBox, opt) {
    ...
    var right$1 = function right(portPosition, elBBox, opt) {
      return labelAttributes(opt, {
        x: 15,
        attrs: {
          '.': {
    ...
    var top$1 = function top(portPosition, elBBox, opt) {
      return labelAttributes(opt, {
        y: -15,
        attrs: {
          '.': {
    ...
    var bottom$1 = function bottom(portPosition, elBBox, opt) {
      return labelAttributes(opt, {
        y: 15,
        attrs: {
          '.': {
    ...
    var outsideOriented = function outsideOriented(portPosition, elBBox, opt) {
      return outsideLayout(portPosition, elBBox, true, opt);
    };
    var outside = function outside(portPosition, elBBox, opt) {
      return outsideLayout(portPosition, elBBox, false, opt);
    };
    var insideOriented = function insideOriented(portPosition, elBBox, opt) {
      return insideLayout(portPosition, elBBox, true, opt);
    };
    var inside = function inside(portPosition, elBBox, opt) {
      return insideLayout(portPosition, elBBox, false, opt);
    };
    var radial = function radial(portPosition, elBBox, opt) {
      return radialLayout(portPosition.difference(elBBox.center()), false, opt);
    };
    var radialOriented = function radialOriented(portPosition, elBBox, opt) {
    ...
    function cloneCells(cells) {
      ...
      toArray(cells).forEach(function (cell) {
        var clone$$1 = cloneMap[cell.id]; // assert(clone exists)
    ...
    function isLinkView$1() {
      return isLinkView(this);
    }
    ...
    var attributesNS = {
      ...
      sourceMarker: {
        qualify: isPlainObject,
        set: function set(marker, refBBox, node, attrs) {
      ...
      text: {
        qualify: function qualify(text, node, attrs) {
          return !attrs.textWrap || !isPlainObject(attrs.textWrap);
        },
        set: function set(text, refBBox, node, attrs) {
          var $node = $(node);
      ...
      textWrap: {
        qualify: isPlainObject,
        set: function set(value, refBBox, node, attrs) {
          // option `width`
      ...
      title: {
        qualify: function qualify(title, node) {
          // HTMLElement title is specified via an attribute (i.e. not an element)
          return node instanceof SVGElement;
        },
        set: function set(title, refBBox, node) {
          var $node = $(node);
      ...
      style: {
        qualify: isPlainObject,
        set: function set(styles, refBBox, node) {
          $(node).css(styles);
        }
      },
      html: {
        set: function set(html, refBBox, node) {
          $(node).html(html + '');
        }
      },
      ...
      refRCircumscribed: {
        set: function set(value, refBBox) {
          var isValuePercentage = isPercentage(value);
          value = parseFloat(value);
      ...
      resetOffset: {
        offset: function offset(val, nodeBBox) {
          return val ? {
            x: -nodeBBox.x,
            y: -nodeBBox.y
          } : {
    ...
    var Cell = Backbone.Model.extend({
      ...
      constructor: function constructor(attributes$$1, options) {
        var defaults$$1;
        ...
        if (defaults$$1 = result(this, 'defaults')) {
          ...
          attrs = merge({}, defaults$$1, attrs); //</custom code>
        }
      ...
      translate: function translate(dx, dy, opt) {
      ...
      remove: function remove(opt) {
        opt = opt || {}; // Store the graph in a variable because `this.graph` won't' be accessbile after `this.trigger('remove', ...)` down below.
        ...
        if (parentCell) parentCell.unembed(this); // Remove also all the cells, which were embedded into this cell
      ...
      parent: function parent(_parent, opt) {
        // getter
        if (_parent === undefined) return this.get('parent'); // setter

        return this.set('parent', _parent, opt);
      },
      embed: function embed(cell, opt) {
      ...
      unembed: function unembed(cell, opt) {
      ...
      getEmbeddedCells: function getEmbeddedCells(opt) {
      ...
      isEmbeddedIn: function isEmbeddedIn(cell, opt) {
      ...
      clone: function clone$$1(opt) {
        opt = opt || {};

        if (!opt.deep) {
          // Shallow cloning.
          var clone$$1 = Backbone.Model.prototype.clone.apply(this, arguments); // We don't want the clone to have the same ID as the original.

          clone$$1.set('id', this.generateId()); // A shallow cloned element does not carry over the original embeds.

          clone$$1.unset('embeds'); // And can not be embedded in any cell
      ...
      prop: function prop(props, value, opt) {
        var delim = '/';
        ...
        if (_isString || Array.isArray(props)) {
          ...
          if (arguments.length > 1) {
            ...
            var attributes$$1 = merge(baseAttributes, update); // Finally, set the property to the updated attributes.
      ...
      removeProp: function removeProp(path, opt) {
        opt = opt || {};
      ...
      attr: function attr(attrs, value, opt) {
        var args = Array.from(arguments);
      ...
      removeAttr: function removeAttr(path, opt) {
      ...
      transition: function transition(path, value, opt, delim) {
        delim = delim || '/';
        var defaults$$1 = {
          duration: 100,
          delay: 10,
      ...
      stopTransitions: function stopTransitions(path, delim) {
        delim = delim || '/';
      ...
      addTo: function addTo(graph, opt) {
      ...
      findView: function findView(paper) {
      ...
      startBatch: function startBatch(name, opt) {
        if (this.graph) {
      ...
      stopBatch: function stopBatch(name, opt) {
        if (this.graph) {
      ...
      getChangeFlag: function getChangeFlag(attributes$$1) {
        var flag = 0;
      ...
      getPointFromConnectedLink: function getPointFromConnectedLink() {
        // To be overridden
        return new g.Point();
      },
      ...
      getAttributeDefinition: function getAttributeDefinition(attrName) {
        var defNS = this.attributes;
        var globalDefNS = attributes;
        return defNS && defNS[attrName] || globalDefNS[attrName];
      },
      define: function define(type, defaults$$1, protoProps, staticProps) {
        protoProps = assign({
          defaults: defaultsDeep({
            type: type
          }, defaults$$1, this.prototype.defaults)
        }, protoProps);
    ...
    var Link = Cell.extend({
      ...
      disconnect: function disconnect(opt) {
        return this.set({
          source: {
            x: 0,
            y: 0
      ...
      source: function source(_source, args, opt) {
        // getter
        ...
        var isCellProvided = _source instanceof Cell;

        if (isCellProvided) {
          // three arguments
          setSource = clone(args) || {};
          setSource.id = _source.id;
          setOpt = opt;
        ...
        var isPointProvided = !isPlainObject(_source);

        if (isPointProvided) {
          // three arguments
          setSource = clone(args) || {};
          setSource.x = _source.x;
          setSource.y = _source.y;
          setOpt = opt;
          return this.set('source', setSource, setOpt);
        } // `source` is an object
        // no checking
      ...
      target: function target(_target, args, opt) {
        // getter
        ...
        var isCellProvided = _target instanceof Cell;

        if (isCellProvided) {
          // three arguments
          setTarget = clone(args) || {};
          setTarget.id = _target.id;
          setOpt = opt;
        ...
        var isPointProvided = !isPlainObject(_target);

        if (isPointProvided) {
          // three arguments
          setTarget = clone(args) || {};
          setTarget.x = _target.x;
          setTarget.y = _target.y;
          setOpt = opt;
          return this.set('target', setTarget, setOpt);
        } // `target` is an object
        // no checking
      ...
      router: function router(name, args, opt) {
        // getter
        if (name === undefined) {
          ...
          if (_typeof(router) === 'object') return clone(router);
          return router; // e.g. a function
        } // setter


        var isRouterProvided = _typeof(name) === 'object' || typeof name === 'function';
      ...
      connector: function connector(name, args, opt) {
        // getter
        if (name === undefined) {
          ...
          if (_typeof(connector) === 'object') return clone(connector);
          return connector; // e.g. a function
        } // setter


        var isConnectorProvided = _typeof(name) === 'object' || typeof name === 'function';
      ...
      label: function label(idx, _label, opt) {
        ...
        return this.prop(['labels', idx], _label, opt);
      },
      labels: function labels(_labels, opt) {
        // getter
        if (arguments.length === 0) {
          _labels = this.get('labels');
          if (!Array.isArray(_labels)) return [];
          return _labels.slice();
        } // setter


        if (!Array.isArray(_labels)) _labels = [];
        return this.set('labels', _labels, opt);
      },
      insertLabel: function insertLabel(idx, label, opt) {
      ...
      appendLabel: function appendLabel(label, opt) {
        return this.insertLabel(-1, label, opt);
      },
      removeLabel: function removeLabel(idx, opt) {
        var labels = this.labels();
      ...
      // Vertices API
      vertex: function vertex(idx, _vertex, opt) {
        ...
        var setVertex = this._normalizeVertex(_vertex);

        return this.prop(['vertices', idx], setVertex, opt);
      },
      vertices: function vertices(_vertices, opt) {
        // getter
        if (arguments.length === 0) {
          _vertices = this.get('vertices');
          if (!Array.isArray(_vertices)) return [];
          return _vertices.slice();
        } // setter


        if (!Array.isArray(_vertices)) _vertices = [];
        var setVertices = [];
      ...
      insertVertex: function insertVertex(idx, vertex, opt) {
      ...
      removeVertex: function removeVertex(idx, opt) {
        var vertices = this.vertices();
        idx = isFinite(idx) && idx !== null ? idx | 0 : -1;
        vertices.splice(idx, 1);
        return this.vertices(vertices, opt);
      },
      _normalizeVertex: function _normalizeVertex(vertex) {
      ...
      // Transformations
      translate: function translate(tx, ty, opt) {
      ...
      scale: function scale$$1(sx, sy, origin, opt) {
        return this.applyToPoints(function (p) {
          return Point(p).scale(sx, sy, origin).toJSON();
        }, opt);
      },
      applyToPoints: function applyToPoints(fn, opt) {
        if (!isFunction(fn)) {
      ...
      getSourcePoint: function getSourcePoint() {
      ...
      getPolyline: function getPolyline() {
      ...
      reparent: function reparent(opt) {
        var newParent;
      ...
      getSourceCell: function getSourceCell() {
        var source = this.source();
        var graph = this.graph;
      ...
      getTargetCell: function getTargetCell() {
        var target = this.target();
        var graph = this.graph;
      ...
      isRelationshipEmbeddedIn: function isRelationshipEmbeddedIn(cell) {
      ...
      endsEqual: function endsEqual(a, b) {
        var portsEqual = a.port === b.port || !a.port && !b.port;
        return a.id === b.id && portsEqual;
      }
    });

    var PortData = function PortData(data) {
    ...
    PortData.prototype = {
      ...
      getPortsByGroup: function getPortsByGroup(groupName) {
        return this.ports.filter(function (port) {
          return port.group === groupName;
        });
      },
      getGroupPortsMetrics: function getGroupPortsMetrics(groupName, elBBox) {
      ...
      _getPortLabelLayout: function _getPortLabelLayout(port, portPosition, elBBox) {
      ...
      _evaluateGroup: function _evaluateGroup(group) {
        return merge(group, {
          position: this._getPosition(group.position, true),
          label: this._getLabel(group, true)
        });
      },
      _evaluatePort: function _evaluatePort(port) {
      ...
      _getZIndex: function _getZIndex(group, port) {
        if (isNumber(port.z)) {
          return port.z;
        }
      ...
      _createPositionNode: function _createPositionNode(group, port) {
        return merge({
          name: 'left',
          args: {}
        }, group.position, {
      ...
      _getPosition: function _getPosition(position, setDefault) {
        var args = {};
        var positionName;
      ...
      _getLabel: function _getLabel(item, setDefaults) {
    ...
    var elementPortPrototype = {
       ...
       */
      getPortsPositions: function getPortsPositions(groupName) {
       ...
       */
      getPortIndex: function getPortIndex(port) {
       ...
       */
      addPort: function addPort(port, opt) {
       ...
       * @returns {joint.dia.Element}
       */
      portProp: function portProp(portId, path, value, opt) {
      ...
      generatePortId: function generatePortId() {
       ...
       * @private
       */
      _isValidPortId: function _isValidPortId(id) {
        return id !== null && id !== undefined && !isObject(id);
      },
      addPorts: function addPorts(ports, opt) {
        if (ports.length) {
      ...
      removePort: function removePort(port, opt) {
        var options = opt || {};
        var ports = assign([], this.prop('ports/items'));
      ...
      removePorts: function removePorts(portsForRemoval, opt) {
        var options;
    ...
    var elementViewPortPrototype = {
       ...
       * @private
       */
      _appendPorts: function _appendPorts(ports, z, refs) {
       ...
       * @private
       */
      _getPortElement: function _getPortElement(port) {
      ...
      findPortNode: function findPortNode(portId, selector) {
       ...
       * @private
       */
      _createPortElement: function _createPortElement(port) {
        var portElement;
        var labelElement;
       ...
       * @private
       */
      _updatePortGroup: function _updatePortGroup(groupName) {
       ...
       * @constructor
       */
      applyPortTransform: function applyPortTransform(element, transformData, initialAngle) {
       ...
       * @private
       */
      _getPortMarkup: function _getPortMarkup(port) {
       ...
       * @private
       */
      _getPortLabelMarkup: function _getPortLabelMarkup(label) {
    ...
    var Element$1 = Cell.extend({
       ...
       * @abstract
       */
      _initializePorts: function _initializePorts() {// implemented in ports.js
      },
      _refreshPorts: function _refreshPorts() {// implemented in ports.js
      },
      isElement: function isElement() {
        return true;
      },
      position: function position(x, y, opt) {
      ...
      translate: function translate(tx, ty, opt) {
        ...
        if (opt.restrictedArea && opt.translateBy === this.id) {
          ...
          var ra = opt.restrictedArea; //- - - - - - - - - - - - -> ra.x + ra.width
          // - - - -> position.x      |
          // -> bbox.x
          //                ▓▓▓▓▓▓▓   |
          //         ░░░░░░░▓▓▓▓▓▓▓
      ...
      size: function size(width, height, opt) {
      ...
      resize: function resize(width, height, opt) {
        opt = opt || {};
        ...
        if (opt.direction) {
          ...
          var bbox = this.getBBox(); // Pick the corner point on the element, which meant to stay on its place before and
          // after the rotation.
          ...
          var origin = Point(center).offset(width / -2, height / -2); // Resize the element (before re-positioning it).
      ...
      fitEmbeds: function fitEmbeds(opt) {
        opt = opt || {}; // Getting the children's size and position requires the collection.
        ...
        if (embeddedCells.length > 0) {
          ...
          var padding = normalizeSides(opt.padding); // Apply padding computed above to the bbox.
      ...
      rotate: function rotate(angle, absolute, origin, opt) {
        if (origin) {
      ...
      getPointFromConnectedLink: function getPointFromConnectedLink(link, endType) {
        // Center of the model
        var bbox = this.getBBox();
    ...
    var wrapWith = function wrapWith(object, methods, wrapper) {
      if (isString(wrapper)) {
        if (!wrappers[wrapper]) {
    ...
    var GraphCells = Backbone.Collection.extend({
      ...
      model: function model(attrs, opt) {
    ...
    var Graph = Backbone.Model.extend({
      _batches: {},
      initialize: function initialize(attrs, opt) {
      ...
      _restructureOnAdd: function _restructureOnAdd(cell) {
        if (cell.isLink()) {
      ...
      _restructureOnRemove: function _restructureOnRemove(cell) {
        if (cell.isLink()) {
      ...
      _restructureOnReset: function _restructureOnReset(cells) {
      ...
      _restructureOnChangeSource: function _restructureOnChangeSource(link) {
      ...
      _restructureOnChangeTarget: function _restructureOnChangeTarget(link) {
      ...
      getOutboundEdges: function getOutboundEdges(node) {
        return this._out && this._out[node] || {};
      },
      // Return all inbound edges for the node. Return value is an object
      // of the form: [edgeId] -> true
      getInboundEdges: function getInboundEdges(node) {
      ...
      _prepareCell: function _prepareCell(cell, opt) {
        var attrs;
      ...
      addCell: function addCell(cell, opt) {
      ...
      addCells: function addCells(cells, opt) {
      ...
      resetCells: function resetCells(cells, opt) {
      ...
      removeCells: function removeCells(cells, opt) {
        if (cells.length) {
      ...
      _removeCell: function _removeCell(cell, collection, options) {
        options = options || {};
      ...
      getConnectedLinks: function getConnectedLinks(model, opt) {
        opt = opt || {};
        var proxy = opt.proxy;
            ...
            if (outbound) {
              forIn(this.getOutboundEdges(cell.id), function (exists, edge) {
                if (!edges[edge]) {
                  ...
                  var targetId = edgeCell.target().id; // if `includeEnclosed` option is falsy, skip enclosed links
      ...
      getNeighbors: function getNeighbors(model, opt) {
        opt || (opt = {});
        var inbound = opt.inbound;
        var outbound = opt.outbound;
      ...
      getSuccessors: function getSuccessors(element, opt) {
        opt = opt || {};
      ...
      cloneSubgraph: function cloneSubgraph(cells, opt) {
      ...
      getSubgraph: function getSubgraph(cells, opt) {
        opt = opt || {};
      ...
      // If `opt.deep` is `true`, take into account embedded elements too.
      // If `opt.breadthFirst` is `true`, use the Breadth-first search algorithm, otherwise use Depth-first search.
      getPredecessors: function getPredecessors(element, opt) {
        opt = opt || {};
      ...
      search: function search(element, iteratee, opt) {
        opt = opt || {};
      ...
      // If `iteratee` explicitly returns `false`, the searching stops.
      bfs: function bfs(element, iteratee, opt) {
        opt = opt || {};
        var visited = {};
        var distance = {};
        var queue = [];
      ...
      dfs: function dfs(element, iteratee, opt, _visited, _distance) {
        opt = opt || {};
      ...
      isPredecessor: function isPredecessor(elementA, elementB) {
      ...
      isNeighbor: function isNeighbor(elementA, elementB, opt) {
        opt = opt || {};
        var inbound = opt.inbound;
        var outbound = opt.outbound;
        ...
        this.getConnectedLinks(elementA, opt).forEach(function (link) {
          var source = link.source();
          var target = link.target(); // Discard if it is a point.
      ...
      disconnectLinks: function disconnectLinks(model, opt) {
        this.getConnectedLinks(model).forEach(function (link) {
          link.set(link.source().id === model.id ? 'source' : 'target', {
            x: 0,
            y: 0
          }, opt);
        });
      ...
      removeLinks: function removeLinks(model, opt) {
        invoke(this.getConnectedLinks(model), 'remove', opt);
      },
      // Find all elements at given point
      findModelsFromPoint: function findModelsFromPoint(p) {
        return this.getElements().filter(function (el) {
          return el.getBBox().containsPoint(p);
        });
      },
      // Find all elements in given area
      findModelsInArea: function findModelsInArea(rect$$1, opt) {
        rect$$1 = rect(rect$$1);
        opt = defaults(opt || {}, {
          strict: false
        });
      ...
      findModelsUnderElement: function findModelsUnderElement(element, opt) {
        opt = defaults(opt || {}, {
          searchBy: 'bbox'
        });
      ...
      getCellsBBox: function getCellsBBox(cells, opt) {
        opt || (opt = {});
      ...
      resizeCells: function resizeCells(width, height, cells, opt) {
      ...
      startBatch: function startBatch(name, data) {
        data = data || {};
      ...
      stopBatch: function stopBatch(name, data) {
        data = data || {};
        this._batches[name] = (this._batches[name] || 0) - 1;
      ...
      hasActiveBatch: function hasActiveBatch(name) {
        if (arguments.length === 0) {
      ...
      validations: {
        multiLinks: function multiLinks(graph, link) {
        ...
        linkPinning: function linkPinning(graph, link) {
    ...
    var View = Backbone.View.extend({
      ...
      renderChildren: function renderChildren(children) {
        ...
        if (children) {
          ...
          var namespace = V$1.namespace[isSVG ? 'xmlns' : 'xhtml'];
      ...
      findAttribute: function findAttribute(attributeName, node) {
      ...
      _createElement: function _createElement(tagName) {
        if (this.svgElement) {
          return document.createElementNS(V$1.namespace.xmlns, tagName);
        } else {
      ...
      _ensureElClassName: function _ensureElClassName() {
        ...
        if (!className) return;
        var prefixedClassName = addClassNamePrefix(className); // Note: className removal here kept for backwards compatibility only
      ...
      init: function init() {// Intentionally empty.
        // This method is meant to be overridden.
      },
      onRender: function onRender() {// Intentionally empty.
        // This method is meant to be overridden.
      },
      ...
      setTheme: function setTheme(theme, opt) {
      ...
      addThemeClassName: function addThemeClassName(theme) {
      ...
      removeThemeClassName: function removeThemeClassName(theme) {
        theme = theme || this.theme;
        var className = this.themeClassNamePrefix + theme;
      ...
      onSetTheme: function onSetTheme(oldTheme, newTheme) {// Intentionally empty.
        // This method is meant to be overridden.
      },
      ...
      onRemove: function onRemove() {// Intentionally empty.
        // This method is meant to be overridden.
      },
      ...
      delegateElementEvents: function delegateElementEvents(element, events, data) {
      ...
      undelegateElementEvents: function undelegateElementEvents(element) {
        $(element).off(this.getEventNamespace());
        return this;
      },
      delegateDocumentEvents: function delegateDocumentEvents(events, data) {
      ...
      eventData: function eventData(evt, data) {
      ...
      isPropagationStopped: function isPropagationStopped(evt) {
      ...
      extend: function extend() {
        ...
        var staticProps = args[1] && assign({}, args[1]) || {}; // Need the real render method so that we can wrap it and call it later.
    ...
    var ToolView = View.extend({
      ...
      configure: function configure(view, toolsView) {
      ...
      simulateRelatedView: function simulateRelatedView(el) {
      ...
      update: function update() {// to be overridden
      }
    });
    var ToolsView = View.extend({
      ...
      configure: function configure(options) {
        ...
        if (!isCellView(relatedView)) return this;
      ...
      focusTool: function focusTool(focusedTool) {
        var tools = this.tools;
      ...
      blurTool: function blurTool(blurredTool) {
        var tools = this.tools;
        if (!tools) return this;
    ...
    var CellView = View.extend({
      ...
      hasFlag: function hasFlag(flag, label) {
        return flag & this.getFlag(label);
      },
      removeFlag: function removeFlag(flag, label) {
        return flag ^ flag & this.getFlag(label);
      },
      getFlag: function getFlag(label) {
      ...
      onAttributesChange: function onAttributesChange(model, opt) {
      ...
      parseDOMJSON: function parseDOMJSON$$1(markup, root) {
      ...
      findBySelector: function findBySelector(selector, root, selectors) {
        root || (root = this.el);
      ...
      getNodeBBox: function getNodeBBox(magnet) {
      ...
      getNodeUnrotatedBBox: function getNodeUnrotatedBBox(magnet) {
        var rect$$1 = this.getNodeBoundingRect(magnet);
        var magnetMatrix = this.getNodeMatrix(magnet);
        var translateMatrix = this.getRootTranslateMatrix();
      ...
      highlight: function highlight(el, opt) {
        el = !el ? this.el : this.$(el)[0] || this.el; // set partial flag if the highlighted element is not the entire view.
      ...
      unhighlight: function unhighlight(el, opt) {
      ...
      findMagnet: function findMagnet(el) {
        var $el = this.$(el);
        var $rootEl = this.$el;
        ...
        do {
          ...
          $el = $el.parent();
        } while ($el.length > 0); // If the overall cell has set `magnet === false`, then return `undefined` to
      ...
      getSelector: function getSelector(el, prevSelector) {
        var selector;
      ...
      getLinkEnd: function getLinkEnd(magnet, x, y, link, endType) {
        var model = this.model;
        var id = model.id;
      ...
      setNodeAttributes: function setNodeAttributes(node, attrs) {
        if (!isEmpty(attrs)) {
      ...
      processNodeAttributes: function processNodeAttributes(node, attrs) {
      ...
      updateRelativeAttributes: function updateRelativeAttributes(node, attrs, refBBox, opt) {
        opt || (opt = {});
        ...
        if (nodeTransform !== undefined || positioned || offseted) {
          ...
          node.setAttribute('transform', V$1.matrixToTransformString(nodeMatrix)); // TODO: store nodeMatrix metrics?
        }
      },
      getNodeScale: function getNodeScale(node, scalableNode) {
      ...
      cleanNodesCache: function cleanNodesCache() {
        this.metrics = {};
      },
      nodeCache: function nodeCache(magnet) {
      ...
      getNodeData: function getNodeData(magnet) {
        var metrics = this.nodeCache(magnet);
        if (!metrics.data) metrics.data = {};
        return metrics.data;
      },
      getNodeBoundingRect: function getNodeBoundingRect(magnet) {
        var metrics = this.nodeCache(magnet);
        if (metrics.boundingRect === undefined) metrics.boundingRect = V$1(magnet).getBBox();
        return new Rect(metrics.boundingRect);
      },
      getNodeMatrix: function getNodeMatrix(magnet) {
        var metrics = this.nodeCache(magnet);
      ...
      getNodeShape: function getNodeShape(magnet) {
        var metrics = this.nodeCache(magnet);
        if (metrics.geometryShape === undefined) metrics.geometryShape = V$1(magnet).toGeometryShape();
        return metrics.geometryShape.clone();
      },
      isNodeConnection: function isNodeConnection(node) {
        return this.model.isLink() && (!node || node === this.el);
      },
      findNodesAttributes: function findNodesAttributes(attrs, root, selectorCache, selectors) {
        ...
        for (var selector in attrs) {
          ...
          for (i = 0, n = selected.length; i < n; i++) {
            ...
            nodeAttrs = attrs[selector]; // "unique" selectors are selectors that referencing a single node (defined by `selector`)
        ...
        for (i = 0, n = mergeIds.length; i < n; i++) {
          ...
          nodeAttrs.attributes = merge.apply(void 0, [{}].concat(_toConsumableArray(nodeAttrs.attributes.reverse())));
        }
      ...
      getEventTarget: function getEventTarget(evt) {
        ...
        var target = evt.target,
            type = evt.type,
            _evt$clientX = evt.clientX,
            clientX = _evt$clientX === void 0 ? 0 : _evt$clientX,
            _evt$clientY = evt.clientY,
            clientY = _evt$clientY === void 0 ? 0 : _evt$clientY;
      ...
      updateDOMSubtreeAttributes: function updateDOMSubtreeAttributes(rootNode, attrs, opt) {
        opt || (opt = {});
        ...
        for (var i = 0, n = relativeItems.length; i < n; i++) {
          ...
          if (!unrotatedRefBBox) {
            ...
            unrotatedRefBBox = bboxCache[refNodeId] = refNode ? V$1(refNode).getBBox({
      ...
      mergeProcessedAttributes: function mergeProcessedAttributes(processedAttrs, roProcessedAttrs) {
      ...
      _toolsView: null,
      hasTools: function hasTools(name) {
        var toolsView = this._toolsView;
        if (!toolsView) return false;
        if (!name) return true;
        return toolsView.getName() === name;
      },
      addTools: function addTools(toolsView) {
        this.removeTools();
      ...
      updateTools: function updateTools(opt) {
        var toolsView = this._toolsView;
      ...
      onToolEvent: function onToolEvent(event) {
        switch (event) {
          case 'remove':
            this.removeTools();
            break;
      ...
      pointerdblclick: function pointerdblclick(evt, x, y) {
        this.notify('cell:pointerdblclick', evt, x, y);
      },
      pointerclick: function pointerclick(evt, x, y) {
        this.notify('cell:pointerclick', evt, x, y);
      },
      contextmenu: function contextmenu(evt, x, y) {
        this.notify('cell:contextmenu', evt, x, y);
      },
      pointerdown: function pointerdown(evt, x, y) {
        if (this.model.graph) {
      ...
      pointermove: function pointermove(evt, x, y) {
        this.notify('cell:pointermove', evt, x, y);
      },
      pointerup: function pointerup(evt, x, y) {
      ...
      mousewheel: function mousewheel(evt, x, y, delta) {
        this.notify('cell:mousewheel', evt, x, y, delta);
      },
      onevent: function onevent(evt, eventName, x, y) {
        this.notify(eventName, evt, x, y);
      },
      onmagnet: function onmagnet() {// noop
      },
      magnetpointerdblclick: function magnetpointerdblclick() {// noop
      },
      magnetcontextmenu: function magnetcontextmenu() {// noop
      },
      ...
      setInteractivity: function setInteractivity(value) {
        this.options.interactive = value;
      }
    }, {
      dispatchToolsEvent: function dispatchToolsEvent(paper, event) {
        if (typeof event === 'string' && isPaper(paper)) {
          paper.trigger('tools:event', event);
        }
      },
      addPresentationAttributes: function addPresentationAttributes(presentationAttributes) {
    ...
    var ElementView = CellView.extend({
       ...
       * @abstract
       */
      _removePorts: function _removePorts() {// implemented in ports.js
      },
       ...
       * @abstract
       */
      _renderPorts: function _renderPorts() {// implemented in ports.js
      },
      className: function className() {
      ...
      initFlag: ['RENDER'],
      UPDATE_PRIORITY: 0,
      confirmUpdate: function confirmUpdate(flag, opt) {
        ...
        if (this.hasFlag(flag, 'RESIZE')) {
          this.resize(opt); // Resize method is calling `update()` internally
       ...
       */
      _initializePorts: function _initializePorts() {},
      update: function update(_$$1, renderingOnlyAttrs) {
      ...
      renderJSONMarkup: function renderJSONMarkup(markup) {
        var doc = this.parseDOMJSON(markup, this.el);
        var selectors = this.selectors = doc.selectors;
        this.rotatableNode = V$1(selectors[this.rotatableSelector]) || null;
        this.scalableNode = V$1(selectors[this.scalableSelector]) || null; // Fragment

        this.vel.append(doc.fragment);
      },
      renderStringMarkup: function renderStringMarkup(markup) {
        var vel = this.vel;
        vel.append(V$1(markup)); // Cache transformation groups
      ...
      sgResize: function sgResize(opt) {
        var model = this.model;
        var angle = model.angle();
        var size = model.size();
      ...
      // -----------------------
      prepareEmbedding: function prepareEmbedding(data) {
        data || (data = {});
      ...
      processEmbedding: function processEmbedding(data) {
        data || (data = {});
        var model = data.model || this.model;
        var paper = data.paper || this.paper;
      ...
      clearEmbedding: function clearEmbedding(data) {
        data || (data = {});
      ...
      finalizeEmbedding: function finalizeEmbedding(data) {
        data || (data = {});
        var candidateView = data.candidateEmbedView;
        var model = data.model || this.model;
        var paper = data.paper || this.paper;
      ...
      onmagnet: function onmagnet(evt, x, y) {
        this.dragMagnetStart(evt, x, y);
      },
      magnetpointerdblclick: function magnetpointerdblclick(evt, magnet, x, y) {
        this.notify('element:magnet:pointerdblclick', evt, magnet, x, y);
      },
      magnetcontextmenu: function magnetcontextmenu(evt, magnet, x, y) {
        this.notify('element:magnet:contextmenu', evt, magnet, x, y);
      },
      // Drag Start Handlers
      dragStart: function dragStart(evt, x, y) {
      ...
      dragMagnetStart: function dragMagnetStart(evt, x, y) {
      ...
      dragLinkStart: function dragLinkStart(evt, magnet, x, y) {
      ...
      addLinkFromMagnet: function addLinkFromMagnet(magnet, x, y) {
        var paper = this.paper;
        var graph = paper.model;
      ...
      drag: function drag(evt, x, y) {
        ...
        var pointerOffset = data.pointerOffset,
            ...
            embedding = data.embedding; // Make sure the new element's position always snaps to the current grid

        var elX = _snapToGrid(x + pointerOffset.x, grid);
        var elY = _snapToGrid(y + pointerOffset.y, grid);
      ...
      dragMagnet: function dragMagnet(evt, x, y) {
        var data = this.eventData(evt);
      ...
      // Drag End Handlers
      dragEnd: function dragEnd(evt, x, y) {
        var data = this.eventData(evt);
        if (data.embedding) this.finalizeEmbedding(data);
      },
      dragMagnetEnd: function dragMagnetEnd(evt, x, y) {
        var data = this.eventData(evt);
        var linkView = data.linkView;
        if (!linkView) return;
      ...
      magnetpointerclick: function magnetpointerclick(evt, magnet, x, y) {
        var paper = this.paper;
    ...
    var normal = function normal(vertices, opt, linkView) {
      return vertices;
    };
    ...
    var oneSide = function oneSide(vertices, opt, linkView) {
      var side = opt.side || 'bottom';
      var padding = normalizeSides(opt.padding || 40); // LinkView contains cached source an target bboxes.
    ...
    function freeJoin(p1, p2, bbox) {
      ...
      if (bbox.containsPoint(p)) p = new Point(p2.x, p1.y); // kept for reference
    ...
    var config$1 = {
      ...
      fallbackRouter: function fallbackRouter(vertices, opt, linkView) {
        if (!isFunction(joint.routers.orthogonal)) {
          throw new Error('Manhattan requires the orthogonal router as default fallback.');
        }

        return joint.routers.orthogonal(vertices, assign({}, config$1, opt), linkView);
      },

      /* Deprecated */
      // a simple route used in situations when main routing method fails
      // (exceed max number of loop iterations, inaccessible)
      fallbackRoute: function fallbackRoute(from, to, opt) {
    ...
    function snapToGrid(point$$1, grid) {
      var source = grid.source;
      var snappedX = _snapToGrid(point$$1.x - source.x, grid.x) + source.x;
      var snappedY = _snapToGrid(point$$1.y - source.y, grid.y) + source.y;
    ...
    function align(point$$1, grid, precision) {
      return round(snapToGrid(point$$1.clone(), grid), precision);
    ...
    function getRectPoints(anchor, bbox, directionList, grid, opt) {
      ...
      var rectPoints = keys.reduce(function (res, key) {
        if (dirList.includes(key)) {
          ...
          var intersectionLine = new Line(anchor, endpoint); // get the farther intersection, in case there are two
          ...
          if (farthestIntersection) {
            var point$$1 = align(farthestIntersection, grid, precision); // if the rectPoint lies inside the bbox, offset it by one more step
    ...
    function router(vertices, opt, linkView) {
      ...
      var sourceBBox = getSourceBBox$1(linkView, opt);
      var targetBBox = getTargetBBox$1(linkView, opt);
      var sourceAnchor = getSourceAnchor$1(linkView, opt); //var targetAnchor = getTargetAnchor(linkView, opt);
      // pathfinding
    ...
    var manhattan = function manhattan(vertices, opt, linkView) {
    ...
    var metro = function metro(vertices, opt, linkView) {
    ...
    var JUMP_TYPES = ['arc', 'gap', 'cubic']; // takes care of math. error for case when jump is too close to end of line
    ...
    function setupUpdating(jumpOverLinkView) {
      var updateList = jumpOverLinkView.paper._jumpOverUpdateList; // first time setup for this paper
    ...
    function createJumps(line$$1, intersections, jumpSize) {
      return intersections.reduce(function (resultLines, point$$1, idx) {
        ...
        var jumpEnd = point(point$$1).move(lastLine.start, +jumpSize); // now try to look at the next intersection point
      ...
      toArray(lines).forEach(function (line$$1, index) {
        if (line$$1.isJump) {
          ...
          if (jumpType === 'arc') {
            ...
            diff = line$$1.start.difference(line$$1.end); // make sure the arc always points up (or right)
    ...
    var jumpover = function jumpover(sourcePoint, targetPoint, route, opt) {
      ...
      if (allLinks.length === 1) {
        return buildPath(createLines(sourcePoint, targetPoint, route), jumpSize, jumpType);
      }
    ...
    var normal$1 = function normal(sourcePoint, targetPoint, route, opt) {
      var raw = opt && opt.raw;
    ...
    var rounded = function rounded(sourcePoint, targetPoint, route, opt) {
      opt || (opt = {});
    ...
    var smooth = function smooth(sourcePoint, targetPoint, route, opt) {
      var raw = opt && opt.raw;
      var path;
    ...
    var LinkView = CellView.extend({
      ...
      onLabelsChange: function onLabelsChange(link, labels, opt) {
      ...
      renderStringMarkup: function renderStringMarkup(markup) {
        ...
        var children = V$1(markup); // custom markup may contain only one children
      ...
      _getLabelMarkup: function _getLabelMarkup(labelMarkup) {
        if (!labelMarkup) return undefined;
        if (Array.isArray(labelMarkup)) return this.parseDOMJSON(labelMarkup, null);
        if (typeof labelMarkup === 'string') return this._getLabelStringMarkup(labelMarkup);
        throw new Error('dia.linkView: invalid label markup');
      },
      _getLabelStringMarkup: function _getLabelStringMarkup(labelMarkup) {
      ...
      _normalizeLabelMarkup: function _normalizeLabelMarkup(markup) {
      ...
      _mergeLabelAttrs: function _mergeLabelAttrs(hasCustomMarkup, labelAttrs, defaultLabelAttrs, builtinDefaultLabelAttrs) {
      ...
      renderVertexMarkers: function renderVertexMarkers() {
        ...
        this.model.vertices().forEach(function (vertex, idx) {
          $markerVertices.append(V$1(markupTemplate(assign({
            idx: idx
          }, vertex))).node);
        });
        return this;
      ...
      renderArrowheadMarkers: function renderArrowheadMarkers() {
        ...
        this._V.sourceArrowhead = V$1(markupTemplate({
          end: 'source'
        }));
        this._V.targetArrowhead = V$1(markupTemplate({
          end: 'target'
        }));
      ...
      update: function update(model, attributes, opt) {
        opt || (opt = {});
        ...
        this.updateTools(opt); // *Deprecated*
      ...
      removeRedundantLinearVertices: function removeRedundantLinearVertices(opt) {
        var link = this.model;
      ...
      getEndView: function getEndView(type) {
        switch (type) {
          case 'source':
      ...
      getEndAnchor: function getEndAnchor(type) {
        switch (type) {
          case 'source':
      ...
      getEndMagnet: function getEndMagnet(type) {
        switch (type) {
          case 'source':
            var sourceView = this.sourceView;
            if (!sourceView) break;
      ...
      updateConnection: function updateConnection(opt) {
        opt = opt || {};
        var model = this.model;
        var route, path;
      ...
      findMarkerPoints: function findMarkerPoints(route, sourcePoint, targetPoint) {
      ...
      findAnchorsOrdered: function findAnchorsOrdered(firstEndType, firstRef, secondEndType, secondRef) {
      ...
      findAnchors: function findAnchors(vertices) {
        var model = this.model;
      ...
      findConnectionPoints: function findConnectionPoints(route, sourceAnchor, targetAnchor) {
        var firstWaypoint = route[0];
      ...
      getAnchor: function getAnchor(anchorDef, cellView, magnet, ref, endType) {
      ...
      getConnectionPoint: function getConnectionPoint(connectionPointDef, view, magnet, line$$1, endType) {
        var connectionPoint;
      ...
      _translateConnectionPoints: function _translateConnectionPoints(tx, ty) {
      ...
      _normalizeLabelPosition: function _normalizeLabelPosition(labelPosition) {
      ...
      updateEndProperties: function updateEndProperties(endType) {
      ...
      _translateAndAutoOrientArrows: function _translateAndAutoOrientArrows(sourceArrow, targetArrow) {
        // Make the markers "point" to their sticky points being auto-oriented towards
        // `targetPosition`/`sourcePosition`. And do so only if there is a markup for them.
        var route = toArray(this.route);

        if (sourceArrow) {
          sourceArrow.translateAndAutoOrient(this.sourcePoint, route[0] || this.targetPoint, this.paper.viewport);
        }

        if (targetArrow) {
          targetArrow.translateAndAutoOrient(this.targetPoint, route[route.length - 1] || this.sourcePoint, this.paper.viewport);
        }
      },
      _getLabelPositionAngle: function _getLabelPositionAngle(idx) {
        var labelPosition = this.model.label(idx).position || {};
        return labelPosition.angle || 0;
      },
      _getLabelPositionArgs: function _getLabelPositionArgs(idx) {
        var labelPosition = this.model.label(idx).position || {};
      ...
      // keep `undefined` or `null` because `{}` means something else
      _mergeLabelPositionArgs: function _mergeLabelPositionArgs(labelPositionArgs, defaultLabelPositionArgs) {
      ...
      addLabel: function addLabel(p1, p2, p3, p4) {
      ...
      addVertex: function addVertex(x, y, opt) {
      ...
      sendToken: function sendToken(token, opt, callback) {
        ...
        vToken.appendTo(this.paper.viewport).animateAlongPath(animationAttributes, connection);
        setTimeout(onAnimationEnd(vToken, callback), duration);
      },
      findRoute: function findRoute(vertices) {
        vertices || (vertices = []);
      ...
      findPath: function findPath(route, sourcePoint, targetPoint) {
      ...
      getPointAtRatio: function getPointAtRatio(ratio) {
        var path = this.path;
        if (!path) return null;
      ...
      getTangentAtRatio: function getTangentAtRatio(ratio) {
        var path = this.path;
        if (!path) return null;
      ...
      getClosestPoint: function getClosestPoint(point$$1) {
        var path = this.path;
        if (!path) return null;
        return path.closestPoint(point$$1, {
          segmentSubdivisions: this.getConnectionSubdivisions()
        });
      },
      getClosestPointLength: function getClosestPointLength(point$$1) {
        var path = this.path;
        if (!path) return null;
        return path.closestPointLength(point$$1, {
          segmentSubdivisions: this.getConnectionSubdivisions()
        });
      },
      getClosestPointRatio: function getClosestPointRatio(point$$1) {
        var path = this.path;
        if (!path) return null;
        return path.closestPointNormalizedLength(point$$1, {
          segmentSubdivisions: this.getConnectionSubdivisions()
        });
      },
      ...
      getLabelPosition: function getLabelPosition(x, y, p3, p4) {
      ...
      _getLabelTransformationMatrix: function _getLabelTransformationMatrix(labelPosition) {
        var labelDistance;
        var labelAngle = 0;
        var args = {};
      ...
      getLabelCoordinates: function getLabelCoordinates(labelPosition) {
      ...
      getVertexIndex: function getVertexIndex(x, y) {
        var model = this.model;
      ...
      onlabel: function onlabel(evt, x, y) {
        this.notifyPointerdown(evt, x, y);
      ...
      // Drag Start Handlers
      dragConnectionStart: function dragConnectionStart(evt, x, y) {
      ...
      dragLabelStart: function dragLabelStart(evt, x, y) {
      ...
      dragVertexStart: function dragVertexStart(evt, x, y) {
      ...
      dragVertexRemoveStart: function dragVertexRemoveStart(evt, x, y) {
      ...
      dragArrowheadStart: function dragArrowheadStart(evt, x, y) {
      ...
      // Drag Handlers
      dragLabel: function dragLabel(evt, x, y) {
        var data = this.eventData(evt);
        var label = {
      ...
      dragVertex: function dragVertex(evt, x, y) {
        var data = this.eventData(evt);
      ...
      dragArrowhead: function dragArrowhead(evt, x, y) {
        var data = this.eventData(evt);
      ...
      // Drag End Handlers
      dragLabelEnd: function dragLabelEnd() {// noop
      },
      dragVertexEnd: function dragVertexEnd() {// noop
      },
      dragArrowheadEnd: function dragArrowheadEnd(evt, x, y) {
        var data = this.eventData(evt);
        var paper = this.paper;
        ...
        this._afterArrowheadMove(data); // mouseleave event is not triggered due to changing pointer-events to `none`.


        this.checkMouseleave(evt);
      },
      dragEnd: function dragEnd() {// noop
      },
      _disallow: function _disallow(data) {
      ...
      _finishEmbedding: function _finishEmbedding(data) {
      ...
      _notifyConnectEvent: function _notifyConnectEvent(data, evt) {
      ...
      _snapArrowhead: function _snapArrowhead(x, y, data) {
        ...
        viewsInArea.forEach(function (view) {
          ...
          if (view.el.getAttribute('magnet') !== 'false') {
            ...
            distance = view.model.getBBox().center().distance(pointer); // the connection is looked up in a circle area by `distance < r`

            if (distance < r && distance < minDistance) {
              if (prevClosestMagnet === view.el || paper.options.validateConnection.apply(paper, data.validateConnectionArgs(view, null))) {
                minDistance = distance;
          ...
          view.$('[magnet]').each(function (index, magnet) {
            ...
            if (distance < r && distance < minDistance) {
              if (prevClosestMagnet === magnet || paper.options.validateConnection.apply(paper, data.validateConnectionArgs(view, magnet))) {
                minDistance = distance;
                data.closestView = view;
      ...
      _snapArrowheadEnd: function _snapArrowheadEnd(data) {
      ...
      _connectArrowhead: function _connectArrowhead(target, x, y, data) {
        ...
        if (data.eventTarget !== target) {
          ...
          if (data.viewUnderPointer) {
            ...
            if (data.magnetUnderPointer && this.paper.options.validateConnection.apply(this.paper, data.validateConnectionArgs(data.viewUnderPointer, data.magnetUnderPointer))) {
      ...
      _connectArrowheadEnd: function _connectArrowheadEnd(data, x, y) {
      ...
      _beforeArrowheadMove: function _beforeArrowheadMove(data) {
      ...
      _afterArrowheadMove: function _afterArrowheadMove(data) {
        if (data.z !== null) {
      ...
      _createValidateConnectionArgs: function _createValidateConnectionArgs(arrowhead) {
      ...
      _markAvailableMagnets: function _markAvailableMagnets(data) {
      ...
      _unmarkAvailableMagnets: function _unmarkAvailableMagnets(data) {
      ...
      startArrowheadMove: function startArrowheadMove(end, opt) {
    ...
    var stroke = {
      ...
      _views: {},
      getHighlighterId: function getHighlighterId(magnetEl, opt) {
        return magnetEl.id + JSON.stringify(opt);
      },
      removeHighlighter: function removeHighlighter(id) {
        if (this._views[id]) {
       ...
       * @param {object=} opt
       */
      highlight: function highlight(cellView, magnetEl, opt) {
        var id = this.getHighlighterId(magnetEl, opt); // Only highlight once.
        ...
        if (cellView.isNodeConnection(magnetEl)) {
          ...
          var highlightMatrix = magnetVel.getTransformToElement(cellView.el); // Add padding to the highlight element.
       ...
       * @param {object=} opt
       */
      unhighlight: function unhighlight(cellView, magnetEl, opt) {
    ...
    var opacity = {
       ...
       * @param {Element} magnetEl
       */
      highlight: function highlight(cellView, magnetEl) {
    ...
    function bboxIntersection(line$$1, view, magnet, opt) {
      ...
      var cp = intersections ? line$$1.start.chooseClosest(intersections) : line$$1.end;
    ...
    function rectangleIntersection(line$$1, view, magnet, opt) {
      ...
      var cp = intersections ? lineWORotation.start.chooseClosest(intersections).rotate(center, -angle) : line$$1.end;
      return offset(cp, line$$1.start, opt.offset);
    }
    ...
    var Paper = View.extend({
      ...
      options: {
        ...
        frozen: false,
        onViewUpdate: function onViewUpdate(view, flag, opt, paper) {
          if (flag & FLAG_INSERT || opt.mounting) return;
          paper.requestConnectedLinksUpdate(view, opt);
        },
        onViewPostponed: function onViewPostponed(view, flag
        /* paper */
        ) {
      ...
      init: function init() {
        ...
        this.$document = $(this.el.ownerDocument); // Highlighters references

        this._highlights = {}; // Render existing cells in the graph

        this.resetViews(model.attributes.cells.models); // Start the Rendering Loop

        if (!this.isFrozen() && this.isAsync()) this.updateViewsAsync();
      },
      _resetUpdates: function _resetUpdates() {
      ...
      onCellAdded: function onCellAdded(cell, _$$1, opt) {
      ...
      onCellRemoved: function onCellRemoved(cell) {
        this.requestViewUpdate(this.findViewByModel(cell), FLAG_REMOVE, 0);
      },
      onCellChange: function onCellChange(cell) {
      ...
      onGraphReset: function onGraphReset(collection, opt) {
        this.removeZPivots();
        this.resetViews(collection.models, opt);
      },
      onGraphSort: function onGraphSort() {
        if (this.model.hasActiveBatch(this.SORT_DELAYING_BATCHES)) return;
        this.sortViews();
      },
      onGraphBatchStop: function onGraphBatchStop(data) {
      ...
      cloneOptions: function cloneOptions() {
        var options = this.options; // This is a fix for the case where two papers share the same options.
        ...
        options.defaultConnector = assign({}, options.defaultConnector); // Return the default highlighting options into the user specified options.

        options.highlighting = defaultsDeep({}, options.highlighting, this.constructor.prototype.options.highlighting); // Default cellView namespace for ES5
      ...
      render: function render() {
        this.$el.empty();
        this.svg = V$1('svg').attr({
          width: '100%',
          height: '100%'
        }).node;
        this.viewport = V$1('g').addClass(addClassNamePrefix('viewport')).node;
        this.defs = V$1('defs').node;
        this.tools = V$1('g').addClass(addClassNamePrefix('tools-container')).node; // Append `<defs>` element to the SVG document. This is useful for filters and gradients.
        // It's desired to have the defs defined before the viewport (e.g. to make a PDF document pick up defs properly).

        V$1(this.svg).append([this.defs, this.viewport, this.tools]);
      ...
      matrix: function matrix(ctm) {
        var viewport = this.viewport; // Getter:
      ...
      clientMatrix: function clientMatrix() {
        return V$1.createSVGMatrix(this.viewport.getScreenCTM());
      },
      requestConnectedLinksUpdate: function requestConnectedLinksUpdate(view, opt) {
      ...
      forcePostponedViewUpdate: function forcePostponedViewUpdate(view, flag) {
      ...
      requestViewUpdate: function requestViewUpdate(view, flag, priority, opt) {
        opt || (opt = {});
      ...
      scheduleViewUpdate: function scheduleViewUpdate(view, type, priority, opt) {
        var updates = this._updates;
        var priorityUpdates = updates.priorities[priority];
        if (!priorityUpdates) priorityUpdates = updates.priorities[priority] = {};
        var currentType = priorityUpdates[view.cid] || 0; // prevent cycling
      ...
      dumpViewUpdate: function dumpViewUpdate(view) {
        if (!view) return 0;
        var updates = this._updates;
        var cid = view.cid;
      ...
      dumpView: function dumpView(view, opt) {
        var flag = this.dumpViewUpdate(view);
        if (!flag) return 0;
        return this.updateView(view, flag, opt);
      },
      updateView: function updateView(view, flag, opt) {
        if (!view) return 0;
      ...
      requireView: function requireView(model, opt) {
        var view = this.findViewByModel(model);
        if (!view) return null;
        this.dumpView(view, opt);
        return view;
      },
      registerUnmountedView: function registerUnmountedView(view) {
        var cid = view.cid;
        var updates = this._updates;
      ...
      registerMountedView: function registerMountedView(view) {
        var cid = view.cid;
        var updates = this._updates;
      ...
      dumpViews: function dumpViews(opt) {
      ...
      updateViews: function updateViews(opt) {
        var stats;
        var updateCount = 0;
        var batchCount = 0;
      ...
      updateViewsAsync: function updateViewsAsync(opt, data) {
        opt || (opt = {});
        data || (data = {
          processed: 0
        });
      ...
      updateViewsBatch: function updateViewsBatch(opt) {
        opt || (opt = {});
      ...
      checkUnmountedViews: function checkUnmountedViews(viewportFn, opt) {
        opt || (opt = {});
        var mountCount = 0;
        if (typeof viewportFn !== 'function') viewportFn = null;
      ...
      checkMountedViews: function checkMountedViews(viewportFn, opt) {
        opt || (opt = {});
        var unmountCount = 0;
      ...
      checkViewport: function checkViewport(opt) {
        var passingOpt = defaults({}, opt, {
      ...
      isExactSorting: function isExactSorting() {
      ...
      setOrigin: function setOrigin(ox, oy) {
      ...
      // the content visible.
      fitToContent: function fitToContent(gridWidth, gridHeight, padding, opt) {
      ...
      scaleContentToFit: function scaleContentToFit(opt) {
        opt || (opt = {});
      ...
      getContentArea: function getContentArea(opt) {
        if (opt && opt.useModelGeometry) {
          var graph = this.model;
          return graph.getCellsBBox(graph.getCells(), {
            includeLinks: true
          }) || new Rect();
        }

        return V$1(this.viewport).getBBox();
      },
      // Return the dimensions of the content bbox in the paper units (as it appears on screen).
      getContentBBox: function getContentBBox(opt) {
      ...
      createViewForModel: function createViewForModel(cell) {
      ...
      renderView: function renderView(cell, opt) {
        var id = cell.id;
      ...
      resetViews: function resetViews(cells, opt) {
        opt || (opt = {});
        cells || (cells = []);
      ...
      sortViewsExact: function sortViewsExact() {
      ...
      insertView: function insertView(view) {
      ...
      addZPivot: function addZPivot(z) {
        z = +z;
        z || (z = 0);
      ...
      removeZPivots: function removeZPivots() {
        var pivots = this._zPivots,
            viewport = this.viewport;

        for (var z in pivots) {
          viewport.removeChild(pivots[z]);
        }

        this._zPivots = {};
      },
      scale: function scale$$1(sx, sy, ox, oy) {
        // getter
        if (sx === undefined) {
      ...
      rotate: function rotate(angle, cx, cy) {
        ...
        if (cx === undefined) {
          var bbox$$1 = this.viewport.getBBox();
          cx = bbox$$1.width / 2;
          cy = bbox$$1.height / 2;
        }
      ...
      findView: function findView($el) {
        var el = isString($el) ? this.viewport.querySelector($el) : $el instanceof $ ? $el[0] : $el;
        var id = this.findAttribute('model-id', el);
        if (id) return this._views[id];
        return undefined;
      },
      // Find a view for a model `cell`. `cell` can also be a string or number representing a model `id`.
      findViewByModel: function findViewByModel(cell) {
        var id = isString(cell) || isNumber(cell) ? cell : cell && cell.id;
        return this._views[id];
      },
      // Find all views at given point
      findViewsFromPoint: function findViewsFromPoint(p) {
        p = new Point(p);
      ...
      findViewsInArea: function findViewsInArea(rect$$1, opt) {
        opt = defaults(opt || {}, {
          strict: false
        });
        ...
        var method = opt.strict ? 'containsRect' : 'intersect';
        return views$$1.filter(function (view) {
          return view && rect$$1[method](view.vel.getBBox({
            target: this.viewport
          }));
        }, this);
      },
      ...
      clientToLocalRect: function clientToLocalRect(x, y, width, height) {
        // allow `x` to be a point and `y` undefined
        var clientRect = new Rect(x, y, width, height);
        var localRect = V$1.transformRect(clientRect, this.clientMatrix().inverse());
        return localRect;
      },
      localToPagePoint: function localToPagePoint(x, y) {
        return this.localToPaperPoint(x, y).offset(this.pageOffset());
      },
      localToPageRect: function localToPageRect(x, y, width, height) {
        return this.localToPaperRect(x, y, width, height).offset(this.pageOffset());
      },
      pageToLocalPoint: function pageToLocalPoint(x, y) {
        var pagePoint = new Point(x, y);
        var paperPoint = pagePoint.difference(this.pageOffset());
        return this.paperToLocalPoint(paperPoint);
      },
      pageToLocalRect: function pageToLocalRect(x, y, width, height) {
      ...
      linkAllowed: function linkAllowed(linkView) {
      ...
      getDefaultLink: function getDefaultLink(cellView, magnet) {
      ...
      // Cell highlighting.
      // ------------------
      resolveHighlighter: function resolveHighlighter(opt) {
        opt = opt || {};
        ...
        var highlighter = paperOpt.highlighterNamespace[name]; // Highlighter validation
      ...
      onCellHighlight: function onCellHighlight(cellView, magnetEl, opt) {
      ...
      onCellUnhighlight: function onCellUnhighlight(cellView, magnetEl, opt) {
        opt = this.resolveHighlighter(opt);
        if (!opt) return;
        var key = opt.name + magnetEl.id + JSON.stringify(opt.options);
      ...
      // Interaction.
      // ------------
      pointerdblclick: function pointerdblclick(evt) {
      ...
      magnetEvent: function magnetEvent(evt, handler) {
      ...
      onmagnet: function onmagnet(evt) {
        this.magnetEvent(evt, function (view, evt, _$$1, x, y) {
          view.onmagnet(evt, x, y);
        });
      },
      magnetpointerdblclick: function magnetpointerdblclick(evt) {
        this.magnetEvent(evt, function (view, evt, magnet, x, y) {
          view.magnetpointerdblclick(evt, magnet, x, y);
        });
      },
      magnetcontextmenu: function magnetcontextmenu(evt) {
        if (this.options.preventContextMenu) evt.preventDefault();
        this.magnetEvent(evt, function (view, evt, magnet, x, y) {
          view.magnetcontextmenu(evt, magnet, x, y);
        });
      },
      onlabel: function onlabel(evt) {
        var labelNode = evt.currentTarget;
      ...
      getPointerArgs: function getPointerArgs(evt) {
        ...
        var _this$snapToGrid = this.snapToGrid(normalizedEvt.clientX, normalizedEvt.clientY),
            x = _this$snapToGrid.x,
            y = _this$snapToGrid.y;

        return [normalizedEvt, x, y];
      },
      delegateDragEvents: function delegateDragEvents(view, data) {
        data || (data = {});
        this.eventData({
          data: data
        }, {
      ...
      _getGriRefs: function _getGriRefs() {
        if (!this._gridCache) {
          this._gridCache = {
            ...
            patterns: {},
            add: function add(id, vel) {
              V$1(this.root.node.childNodes[0]).append(vel);
              this.patterns[id] = vel;
              this.root.append(V$1('rect', {
                width: '100%',
                height: '100%',
                fill: 'url(#' + id + ')'
              }));
      ...
      setGrid: function setGrid(drawGrid) {
        this.clearGrid();
      ...
      _resolveDrawGridOption: function _resolveDrawGridOption(opt) {
      ...
      drawGrid: function drawGrid(opt) {
      ...
      updateBackgroundImage: function updateBackgroundImage(opt) {
        opt = opt || {};
      ...
      drawBackgroundImage: function drawBackgroundImage(img, opt) {
      ...
      drawBackground: function drawBackground(opt) {
        opt = opt || {};
      ...
      // Paper definitions.
      // ------------------
      isDefined: function isDefined(defId) {
        return !!this.svg.getElementById(defId);
      },
      defineFilter: function defineFilter(filter$$1) {
        if (!isObject(filter$$1)) {
          throw new TypeError('dia.Paper: defineFilter() requires 1. argument to be an object.');
        }

        var filterId = filter$$1.id;
        var name = filter$$1.name; // Generate a hash code from the stringified filter definition. This gives us
      ...
      defineGradient: function defineGradient(gradient) {
      ...
      defineMarker: function defineMarker(marker) {
        if (!isObject(marker)) {
      ...
      isPaper: function isPaper$$1() {
        return true;
      }
    }, {
      sorting: sortingTypes,
      backgroundPatterns: {
        flipXy: function flipXy(img) {
          // d b
          // q p
        ...
        watermark: function watermark(img, opt) {
          //   d
          // d
          opt = opt || {};
          var imgWidth = img.width;
    ...
    var DirectedGraph = {
      exportElement: function exportElement(element) {
        // The width and height of the element.
        return element.size();
      },
      exportLink: function exportLink(link) {
      ...
      importElement: function importElement(opt, v, gl) {
      ...
      importLink: function importLink(opt, edgeObj, gl) {
        var link = this.getCell(edgeObj.name);
        var glEdge = gl.edge(edgeObj);
        var points = glEdge.points || []; // check the `setLinkVertices` here for backwards compatibility
      ...
      layout: function layout(graphOrCells, opt) {
        ...
        var glGraph = DirectedGraph.toGraphLib(graph, {
          ...
          setEdgeName: function setEdgeName(link) {
        ...
        return Rect(marginX, marginY, Math.abs(glSize.width - 2 * marginX), Math.abs(glSize.height - 2 * marginY));
      },
      fromGraphLib: function fromGraphLib(glGraph, opt) {
        opt = opt || {};
      ...
      toGraphLib: function toGraphLib(graph, opt) {
        opt = opt || {};
    ...
    var version = '2.2.1';
    var setTheme = function setTheme(theme, opt) {
      opt = opt || {};
      invoke(views, 'setTheme', theme, opt); // Update the default theme on the view prototype.
    ...
    var env = {
      ...
      addTest: function addTest(name, fn) {
    ...
    var TextBlock = Generic.define('basic.TextBlock', {
      ...
      updateSize: function updateSize(cell, size) {
      ...
      updateContent: function updateContent(cell, content) {
    ...
    var TextBlockView = ElementView.extend({
      ...
      updateContent: function updateContent(cell, renderingOnlyAttrs) {
    ...
    var TextBlock$1 = Element$1.define('standard.TextBlock', {
      ...
      attributes: {
        text: {
          ...
          position: function position(text, refBBox, node) {
            // No foreign object
    ...
    var Model = Generic.define('devs.Model', {
      ...
      updatePortItems: function updatePortItems(model, changed, opt) {
      ...
      createPortItem: function createPortItem(group, port) {
        return {
          id: port,
          group: group,
          attrs: {
      ...
      createPortItems: function createPortItems(group, ports) {
        return toArray(ports).map(this.createPortItem.bind(this, group));
      },
      _addGroupPort: function _addGroupPort(port, group, opt) {
        var ports = this.get(group);
        return this.set(group, Array.isArray(ports) ? ports.concat(port) : [port], opt);
      },
      addOutPort: function addOutPort(port, opt) {
        return this._addGroupPort(port, 'outPorts', opt);
      },
      addInPort: function addInPort(port, opt) {
        return this._addGroupPort(port, 'inPorts', opt);
      },
      _removeGroupPort: function _removeGroupPort(port, group, opt) {
        return this.set(group, without(this.get(group), port), opt);
      },
      removeOutPort: function removeOutPort(port, opt) {
        return this._removeGroupPort(port, 'outPorts', opt);
      },
      removeInPort: function removeInPort(port, opt) {
        return this._removeGroupPort(port, 'inPorts', opt);
      },
      _changeGroup: function _changeGroup(group, properties, opt) {
        return this.prop('ports/groups/' + group, isObject(properties) ? properties : {}, opt);
      },
      changeInGroup: function changeInGroup(properties, opt) {
        return this._changeGroup('in', properties, opt);
      },
      changeOutGroup: function changeOutGroup(properties, opt) {
    ...
    var Repeater = Gate11.define('logic.Repeater', {
      ...
      operation: function operation(input) {
        return input;
      }
    });
    ...
    var Or = Gate21.define('logic.Or', {
      ...
      operation: function operation(input1, input2) {
    ...
    var Line$1 = Link.define('erd.Line', {}, {
      cardinality: function cardinality(value) {
        this.set('labels', [{
          position: -20,
          attrs: {
            text: {
    ...
    var Association = Link.define('uml.Association'); // Statechart
    ...
    var index$2 = ({
        basic: basic,
        standard: standard,
        devs: joint_shapes_devs,
        logic: joint_shapes_logic,
        chess: joint_shapes_chess,
        erd: joint_shapes_erd,
        fsa: joint_shapes_fsa,
        org: joint_shapes_org,
        pn: joint_shapes_pn,
        uml: joint_shapes_uml
    });
    ...
    var Vertices = ToolView.extend({
      ...
      updateHandles: function updateHandles() {
        var relatedView = this.relatedView;
        var vertices = relatedView.model.vertices();
      ...
      startHandleListening: function startHandleListening(handle) {
        var relatedView = this.relatedView;
      ...
      getNeighborPoints: function getNeighborPoints(index) {
      ...
      onHandleWillChange: function onHandleWillChange(_handle, evt) {
        this.focus();
        var relatedView = this.relatedView,
        ...
        if (!options.stopPropagation) relatedView.notifyPointerdown.apply(relatedView, _toConsumableArray(relatedView.paper.getPointerArgs(evt)));
      },
      onHandleChanging: function onHandleChanging(handle, evt) {
        var options = this.options,
            linkView = this.relatedView;
        var index = handle.options.index;

        var _linkView$paper$getPo = linkView.paper.getPointerArgs(evt),
            _linkView$paper$getPo2 = _slicedToArray(_linkView$paper$getPo, 3),
            normalizedEvent = _linkView$paper$getPo2[0],
            x = _linkView$paper$getPo2[1],
            y = _linkView$paper$getPo2[2];
      ...
      onHandleChanged: function onHandleChanged(_handle, evt) {
        var options = this.options,
            linkView = this.relatedView;
        ...
        var _linkView$paper$getPo3 = linkView.paper.getPointerArgs(evt),
            _linkView$paper$getPo4 = _slicedToArray(_linkView$paper$getPo3, 3),
            normalizedEvt = _linkView$paper$getPo4[0],
            x = _linkView$paper$getPo4[1],
            y = _linkView$paper$getPo4[2];

        linkView.checkMouseleave(normalizedEvt);
        if (!options.stopPropagation) linkView.notifyPointerup(normalizedEvt, x, y);
      },
      snapVertex: function snapVertex(vertex, index) {
      ...
      onHandleRemove: function onHandleRemove(handle, evt) {
        var index = handle.options.index;
        var linkView = this.relatedView;
      ...
      onPathPointerDown: function onPathPointerDown(evt) {
        evt.stopPropagation();
        evt.preventDefault();
    ...
    var SegmentHandle = View.extend({
      ...
      position: function position(x, y, angle, view) {
    ...
    var Segments = ToolView.extend({
      ...
      renderHandle: function renderHandle(vertex, nextVertex) {
        var handle = new this.options.handleClass({
          paper: this.paper
        });
        handle.render();
      ...
      shiftHandleIndexes: function shiftHandleIndexes(value) {
        var handles = this.handles;
      ...
      resetAnchor: function resetAnchor(type, anchor) {
      ...
      snapHandle: function snapHandle(handle, position, data) {
        var index = handle.options.index;
        var linkView = this.relatedView;
        var link = linkView.model;
        var vertices = link.vertices();
      ...
      onHandleChangeStart: function onHandleChangeStart(handle, evt) {
        var index = handle.options.index;
        var handles = this.handles;
        if (!Array.isArray(handles)) return;
        ...
        relatedView.model.startBatch('segment-move', {
          ui: true,
          tool: this.cid
        });
      },
      onHandleChangeEnd: function onHandleChangeEnd(_handle, evt) {
        var linkView = this.relatedView;
        ...
        this.render();
        this.blur();
        linkView.model.stopBatch('segment-move', {
          ui: true,
          tool: this.cid
        });
        linkView.checkMouseleave(normalizeEvent(evt));
      },
      updateHandle: function updateHandle(handle, vertex, nextVertex, offset) {
    ...
    var Button = ToolView.extend({
      ...
      update: function update() {
        ...
        var matrix = V$1.createSVGMatrix().translate(position.x, position.y).rotate(angle).translate(0, this.options.offset || 0);
    ...
    var Remove = Button.extend({
      ...
      options: {
        distance: 60,
        offset: 0,
        action: function action(evt, view, tool) {
          view.model.remove({
            ui: true,
            tool: tool.cid
          });
    ...
    var Anchor = ToolView.extend({
      ...
      toggleArea: function toggleArea(visible) {
      ...
      resetAnchor: function resetAnchor(anchor) {
        var type = this.type;
        var relatedModel = this.relatedView.model;
